...
- **Markdown 格式**:mdformat 按 `.mdformat.toml` 治理(pinned 1.0.0,CI `docs-format` job 检查 git 跟踪的 `*.md`);本地 `uv run mdformat <files>`。
- **本手册双名同源**:`AGENTS.md` 是真身,`CLAUDE.md` 是指向它的 symlink(兼容只认 CLAUDE.md 的工具);只编辑 `AGENTS.md`,不要把 `CLAUDE.md` 换回普通文件造成两份分叉。
