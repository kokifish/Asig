...
- **Done Notification**:别的状态刚转入 Done 的 **30 秒内**,灯短暂变成**深绿、快速呼吸**(菜单栏用 💚 表示),提示「刚完成,回来看」;之后回退为绿色波纹。
- 多个会话同时存在时,灯显示最需要关注的那一个(优先级数字大者覆盖:**红 > 琥珀 > 紫 > 黄 > 绿**)。
- `NeedsDeci` / `Error` / `Offline` 一旦出现即**保持**,只有重新 `Working` 或 `Done` 才解除——不会因超时自动变。

## 动效说明

灯的颜色之外,还有 4 种动效(括号内为通用英文 / 代码里的名字):

| 动效 | 英文 | 看起来 |
|---|---|---|
| 常亮 | Steady(solid) | 纯色不变 |
| 呼吸 | Pulse(breathing) | 缓慢明暗往复 |
| 明灭 | Blink(blink / flash) | 一亮一灭地闪 |
| 波纹 | Ripple(sonar) | 一圈环从中心扩散并淡出 |

- 「快闪」「慢闪」是**同一个 Blink**,只是快慢不同(Error 比 NeedsDeci 快)。
- 每个状态的动效、颜色、快慢都能在**设置**里单独调,即时生效、自动保存。
