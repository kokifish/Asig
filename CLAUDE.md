...
- **`screencapture -l<windowID>` 单窗口截图不合成桌面背景**:液态玻璃 / vibrancy 会退化成纯色(看着像没生效)。验证玻璃 / 材质效果要**全屏 `screencapture`(不带 `-l`)再按窗口 bounds 裁剪**;单窗口截图只能看几何对齐。
...
## 治理
- **完成后必须自测**:每完成一项功能改进,先跑相关自测再报告完成(自测不过 = 未完成)。至少:
  `cargo fmt --all -- --check` + `cargo clippy --workspace --all-targets -- -D warnings` + `cargo test --workspace`(单元测试);
  涉及 UI / 运行时的改动还要 `./scripts/make-app.sh` 重新打包,**像素级截图确认**(全屏 `screencapture` 再按窗口 bounds 裁窗,别用单窗口 `-l<wid>` —— 见「macOS / AppKit 坑」;也别只靠目测或图像分析器),并 **`open build/Asig.app` 启动最新版本** 供实测核对关键点。
- **DEV.md 是唯一权威手册,未经用户明确允许不可改**;改了设计要同步给出 DEV.md 的同步方案
  - DEV.md **未明确的**:先问用户再动,别猜。
  - DEV.md **与代码脱节 / 过时 / 不准的**:给修改方案并问用户,不自作主张改代码或改 DEV.md。
...
- **提交规则**:
  - `git commit` / `git push` 须**用户明确指令**才执行(`git add` 可自行)。
  - 提交信息**全英文**、正文 **≤ 8 行**、不带测试结果类描述。
  - **不带任何 Claude / Co-Authored-By 字样**。
- README.md: DEV.md的简化版，面向用户